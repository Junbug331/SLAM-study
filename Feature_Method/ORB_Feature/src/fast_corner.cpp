...
// 32 bit unsigned bit, will have 8, 8 x 32 = 256
 ...
 *
 * If a keypoint goes outside the image boundary (8 pixels), descriptors will be
 * empty
 */
void ComputeORB(const cv::Mat &img, std::vector<cv::KeyPoint> &keypoints,
...
int main() {
  fs::path first_img_path = fs::path(RES_DIR) / "1.png";
  fs::path second_img_path = fs::path(RES_DIR) / "2.png";

  // load images
  cv::Mat first_img = cv::imread(first_img_path, 0);
  cv::Mat second_img = cv::imread(second_img_path, 0);
  assert(first_img.empty() && second_img.empty());
  ...
  cv::FAST(first_img, keypoints1, 40);

  // ORB-BRIEF
  std::vector<DescType> descriptor1;
  ComputeORB(first_img, keypoints1, descriptor1);

  // same for the second image
  std::vector<cv::KeyPoint> keypoints2;
  std::vector<DescType> descriptor2;
  cv::FAST(second_img, keypoints2, 40);
  ComputeORB(second_img, keypoints2, descriptor2);

  spdlog::info("extract ORB cost: {}", sw);

  // find matches
  std::vector<cv::DMatch> matches;
  sw.reset();
  BfMatch(descriptor1, descriptor2, matches);
  spdlog::info("match ORB cost = {}", sw);
  spdlog::info("matches: {}", matches.size());

  // plot the matches
  cv::Mat img_show;
  cv::drawMatches(first_img, keypoints1, second_img, keypoints2, matches,
                  img_show);
...
void ComputeORB(const cv::Mat &img, std::vector<cv::KeyPoint> &keypoints,
  ...
  for (auto &kp : keypoints) {
    ...
    // first moment of block of the image around a keypoint
    // image block -8 ~ 8: 16
